...
vi.mock('./gh-video-compressor.worker', () => ({
...
import { createGhAttachmentService } from './gh-attachment.service';
import { validateFileMime } from './gh-video-compressor.worker';
...
function makeService(over: Record<string, any> = {}) {
  ...
  const findParent =
    over.findParent ??
    vi.fn().mockResolvedValue({ id: 'gh-report-1', deletedAt: null, location: { rtuppId: 'rtupp-2' } });
  const svc = createGhAttachmentService({
    label: 'Inspeksi GH',
    parentIdField: 'laporanInspeksiGhId',
    findParent,
    attachmentDelegate: attachmentDelegate as never,
    uploadDir: '/tmp/gh-attach-test',
  });
  return { svc, attachmentDelegate, findParent };
}

describe('gh-attachment.service — scope & upload', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('upload menolak parent di luar RTUPP → ForbiddenError', async () => {
    const { svc } = makeService({
      findParent: vi.fn().mockResolvedValue({ id: 'gh-1', deletedAt: null, location: { rtuppId: 'rtupp-9' } }),
    });
    await expect(
      svc.upload('gh-1', GiAttachmentCategory.FOTO, '/tmp/x.png', 'x.png', 100, actor, scope),
  ...
  it('upload menolak parent tidak ditemukan → NotFoundError', async () => {
    ...
    await expect(
      svc.upload('gh-x', GiAttachmentCategory.FOTO, '/tmp/x.png', 'x.png', 100, actor, scope),
  ...
  it('upload menolak mime tidak valid', async () => {
    ...
    const { svc } = makeService();
    await expect(
      svc.upload('gh-report-1', GiAttachmentCategory.FOTO, '/tmp/x.exe', 'x.exe', 100, actor, scope),
    ).rejects.toBeInstanceOf(AppError);
  });

  it('upload FOTO tersimpan READY dengan FK parentIdField benar', async () => {
    const { svc, attachmentDelegate } = makeService();
    const att = await svc.upload('gh-report-1', GiAttachmentCategory.FOTO, '/tmp/x.png', 'x.png', 100, actor, scope);
    expect(att.status).toBe(GiAttachmentStatus.READY);
    expect(att.laporanInspeksiGhId).toBe('gh-report-1');
  ...
  it('upload VIDEO tersimpan PROCESSING', async () => {
    ...
    const { svc } = makeService();
    const att = await svc.upload('gh-report-1', GiAttachmentCategory.VIDEO, '/tmp/x.mp4', 'x.mp4', 100, actor, scope);
  ...
  it('list menolak akses lintas-RTUPP', async () => {
    ...
    await expect(svc.list('gh-1', actor, scope)).rejects.toBeInstanceOf(ForbiddenError);
  });
    ...
    const { svc, attachmentDelegate } = makeService({
      attachmentDelegate: {
        findFirst: vi.fn().mockResolvedValue({
          ...
          originalName: 'f.png',
          mimeType: 'image/png',
          laporanInspeksiGh: { location: { rtuppId: 'rtupp-9' } },
        }),
      },
    });
    ...
    const { svc } = makeService({
      attachmentDelegate: {
        findFirst: vi.fn().mockResolvedValue({
          ...
          originalName: 'f.mp4',
          mimeType: 'video/mp4',
          laporanInspeksiGh: { location: { rtuppId: 'rtupp-2' } },
        }),
      },
    });
