...
import {
  laporanInspeksiMpRepository,
  LaporanInspeksiMpRepository,
  type ImportedMpRecord,
} from './laporan-inspeksi-mp.repository';
import {
  BusinessRuleError,
  ForbiddenError,
  NotFoundError,
...
// statusRc dibaca dari entri kubikel. Mendukung kubikel berupa array (per-gardu;
...
// Kubikel array-aware submit-gate (dipakai Inspeksi & HAR MP):
//  - minimal 1 entri gardu;
//  - tiap entri wajib: namaGardu + statusCubicle + statusRc;
//  - relayDetail TIDAK wajib.
// Toleran objek tunggal (kompat lama) dengan membungkusnya jadi 1-array.
export function assertKubikelSubmittableMp(kubikelVal: unknown, missing: string[]) {
  const arr = Array.isArray(kubikelVal)
    ...
    : isObj(kubikelVal)
      ...
      : [];
  if (arr.length === 0) {
    missing.push('Kubikel (min. 1 gardu)');
    return;
  }
  ...
  arr.forEach((e, i) => {
    ...
    if (!has(e.namaGardu)) missing.push(`Kubikel #${no}: Nama Gardu`);
...
export class LaporanInspeksiMpService {
  constructor(private readonly repo: LaporanInspeksiMpRepository = laporanInspeksiMpRepository) {}

  async list(query: ListLaporanInspeksiMpQuery, actor: Actor, scope: TenantScope) {
  ...
  // Baris "Laporan Inspeksi MP" semu dari data historis import Excel — tidak
  // pernah punya WO/status alur (DRAFT/SUBMITTED/…), maka `status: null` dan FE
  // membedakan render lewat `isImported`.
  private mapImportedRow(r: ImportedMpRecord) {
    return {
      id: r.id,
      workOrderId: null,
  ...
  async getById(id: string, actor: Actor, scope: TenantScope) {
    ...
    if (!report) throw new NotFoundError('Laporan Inspeksi MP tidak ditemukan');
    if (isFieldOfficer(actor.role) && report.inspectorId !== actor.userId) {
      throw new ForbiddenError('Laporan Inspeksi MP ini bukan milik Anda');
    }
    return report;
  }
  ...
  // Pencarian Gardu Distribusi (dropdown), fail-closed RTUPP scope.
  async searchGardu(query: string, scope: TenantScope, limit = 20) {
    const gardus = await this.repo.searchGardu(query, scope, limit);
    return gardus.map((g) => ({
      id: g.id,
      code: g.code,
      name: g.name,
      up3: g.up3,
      feederId: g.supplyFeeder?.id ?? null,
      feederName: g.supplyFeeder?.feederName ?? null,
    }));
  }
  ...
  async create(input: CreateLaporanInspeksiMpInput, actor: Actor, scope: TenantScope) {
  ...
  async update(id: string, input: UpdateLaporanInspeksiMpInput, actor: Actor, scope: TenantScope) {
    const current = await this.getById(id, actor, scope);
    assertWoNotRejected(current.workOrder?.status);
    if (!['DRAFT', 'REJECTED'].includes(current.status)) {
      throw new BusinessRuleError(`Laporan Inspeksi MP berstatus ${current.status} tidak dapat diubah`);
    }
  ...
  async submit(id: string, actor: Actor, scope: TenantScope) {
    ...
    assertWoNotRejected(current.workOrder?.status);
    if (!['DRAFT', 'REJECTED'].includes(current.status)) {
      throw new BusinessRuleError(`Laporan Inspeksi MP berstatus ${current.status} tidak dapat dikirim`);
    }
  ...
  async decide(id: string, input: DecideLaporanInspeksiMpInput, actor: Actor, scope: TenantScope) {
    const current = await this.getById(id, actor, scope);
    if (current.workOrderId) {
      throw new BusinessRuleError('Approval Laporan Inspeksi MP tertaut Work Order dilakukan di Work Order terkait');
    }
...
export const laporanInspeksiMpService = new LaporanInspeksiMpService();
