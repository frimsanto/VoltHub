...
import { LaporanHarGhService } from './laporan-har-gh.service';
import { createLaporanHarGhSchema } from './laporan-har-gh.validation';
...
const completeReport = (over: Record<string, unknown> = {}) => ({
  ...
  status: 'DRAFT',
  statusRc: 'SIAP RC',
  location: { code: 'GH0004', rtuppId: 'rtupp-3' },
  supplyTr: ok(),
  rectifier: ok(),
  baterai: ok(),
  rtu: ok(),
...
function makeRepo(over: Record<string, any> = {}) {
  return {
    workOrderById: vi.fn().mockResolvedValue({
      ...
      locationId: 'loc-1',
      feederId: null,
      location: { up3: 'UP3 X', rtuppId: 'rtupp-3', locationType: 'GH' },
      team: { name: 'PT ARHADI' },
    }),
    findById: vi.fn(),
    create: vi.fn().mockImplementation(async (data: any) => ({ id: 'har-1', ...data })),
...
const svc = (repo: any) => new LaporanHarGhService(repo as any);

describe('HAR GH — workOrderId WAJIB + identitas dari WO', () => {
  it('schema menolak create tanpa workOrderId', () => {
    expect(createLaporanHarGhSchema.safeParse({ reportDate: '2024-11-05' }).success).toBe(false);
  });
  ...
  it('WO di luar RTUPP → 403', async () => {
    const repo = makeRepo({
      workOrderById: vi.fn().mockResolvedValue({
        ...
        locationId: 'loc-9',
        feederId: null,
        location: { up3: 'Y', rtuppId: 'rtupp-9', locationType: 'GH' },
        team: { name: 'Z' },
      }),
    });
    await expect(
...
describe('HAR GH — submit array-aware kubikel', () => {
  ...
  it('menolak submit bila WO tertaut berstatus REJECTED ("Tidak Sesuai")', async () => {
    ...
    await expect(svc(repo).submit('har-1', petugas, scope)).rejects.toThrowError(BusinessRuleError);
