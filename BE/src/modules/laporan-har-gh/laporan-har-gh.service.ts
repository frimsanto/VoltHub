...
import {
  laporanHarGhRepository,
  LaporanHarGhRepository,
  type ImportedHarGhRecord,
} from './laporan-har-gh.repository';
import {
  BusinessRuleError,
  ForbiddenError,
  NotFoundError,
...
import { assertKubikelSubmittable } from '../laporan-inspeksi-gh/laporan-inspeksi-gh.service';
...
// statusRc dibaca dari entri kubikel. Mendukung kubikel array (per-penyulang; entri
...
export class LaporanHarGhService {
  constructor(private readonly repo: LaporanHarGhRepository = laporanHarGhRepository) {}

  async list(query: ListLaporanHarGhQuery, actor: Actor, scope: TenantScope) {
  ...
  // Baris "Laporan HAR GH" semu dari data historis import Excel — tidak pernah
  // punya WO/status alur (DRAFT/SUBMITTED/…), maka `status: null` dan FE
  // membedakan render lewat `isImported`.
  private mapImportedRow(r: ImportedHarGhRecord) {
    return {
      id: r.id,
      workOrderId: null,
  ...
  async getById(id: string, actor: Actor, scope: TenantScope) {
    ...
    if (!report) throw new NotFoundError('Laporan HAR GH tidak ditemukan');
    if (isFieldOfficer(actor.role) && report.inspectorId !== actor.userId) {
      throw new ForbiddenError('Laporan HAR GH ini bukan milik Anda');
    }
    return report;
  }
  ...
  async create(input: CreateLaporanHarGhInput, actor: Actor, scope: TenantScope) {
  ...
  async update(id: string, input: UpdateLaporanHarGhInput, actor: Actor, scope: TenantScope) {
    const current = await this.getById(id, actor, scope);
    assertWoNotRejected(current.workOrder?.status);
    if (!['DRAFT', 'REJECTED'].includes(current.status)) {
      throw new BusinessRuleError(`Laporan HAR GH berstatus ${current.status} tidak dapat diubah`);
    }
  ...
  async submit(id: string, actor: Actor, scope: TenantScope) {
    ...
    assertWoNotRejected(current.workOrder?.status);
    if (!['DRAFT', 'REJECTED'].includes(current.status)) {
      throw new BusinessRuleError(`Laporan HAR GH berstatus ${current.status} tidak dapat dikirim`);
    }
    ...
    // HAR GH korektif — SATU jalur via WO, TANPA resultMapping (tak menyentuh
    // hasilRC WO), sama pola LaporanHarGi. Notifikasi submit dikirim oleh WO.
    await workOrderService.onLinkedReportSubmitted(current.workOrderId, {}, actor, scope);

    return updated;
  }

  async decide(id: string, input: DecideLaporanHarGhInput, actor: Actor, scope: TenantScope) {
    const current = await this.getById(id, actor, scope);
    if (current.workOrderId) {
      throw new BusinessRuleError('Approval Laporan HAR GH tertaut Work Order dilakukan di Work Order terkait');
    }
...
export const laporanHarGhService = new LaporanHarGhService();
