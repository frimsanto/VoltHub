// VoltHub — Lampiran Laporan MP (SLD/Logger/Foto/Video), dipakai Inspeksi MP & HAR
// MP. Mirror GhAttachments.tsx 1:1, diparameterisasi lewat `basePath` + `parentId`
// (endpoint attachment BE mirror pola GH: /mp/inspeksi/... atau /mp/har/...).
// ADITIF — tidak mengubah komponen GH/GI.
...
export interface MpAttachmentsProps {
  /** "/v1/mp/inspeksi" atau "/v1/mp/har". */
  basePath: string;
  parentId?: string;
  readOnly?: boolean;
}

export function MpAttachments({ basePath, parentId, readOnly = false }: MpAttachmentsProps) {
  ...
  return (
    <Card className="w-full">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
        <CardTitle className="text-base font-bold">Dokumen &amp; Lampiran Teknis MP</CardTitle>
      ...
      <CardContent className="space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 pb-3 border-b">
            ...
            {!readOnly && (
              <div className="relative">
                <input type="file" id="mp-sld-upload" accept="image/png, image/jpeg, image/jpg" className="hidden" disabled={uploadingCategory !== null} onChange={(e) => handleUpload(e, "SLD")} />
                <Button size="sm" variant="outline" asChild disabled={uploadingCategory !== null}>
                  <label htmlFor="mp-sld-upload" className="cursor-pointer flex items-center gap-1"><Upload className="h-3.5 w-3.5" /> Unggah SLD</label>
                </Button>
              </div>
            )}
          </div>
        ...
        <div className="rounded-lg border p-4">
          <div className="flex flex-wrap items-center justify-between gap-2 pb-3 border-b">
            ...
            {!readOnly && (
              <div className="relative">
                <input type="file" id="mp-logger-upload" accept=".xls, .xlsx, .csv" className="hidden" disabled={uploadingCategory !== null} onChange={(e) => handleUpload(e, "LOGGER")} />
                <Button size="sm" variant="outline" asChild disabled={uploadingCategory !== null}>
                  <label htmlFor="mp-logger-upload" className="cursor-pointer flex items-center gap-1"><Upload className="h-3.5 w-3.5" /> Unggah Logger</label>
                </Button>
              </div>
            )}
          </div>
        ...
        <div className="rounded-lg border p-4">
          <div className="flex flex-wrap items-center justify-between gap-2 pb-3 border-b">
            ...
            {!readOnly && (
              <div className="relative">
                <input type="file" id="mp-photo-upload" accept="image/png, image/jpeg, image/jpg" multiple className="hidden" disabled={uploadingCategory !== null || photoFiles.length >= 20} onChange={(e) => handleUpload(e, "FOTO")} />
                <Button size="sm" variant="outline" asChild disabled={uploadingCategory !== null || photoFiles.length >= 20}>
                  <label htmlFor="mp-photo-upload" className="cursor-pointer flex items-center gap-1"><Upload className="h-3.5 w-3.5" /> Unggah Foto</label>
                </Button>
              </div>
            )}
          </div>
        ...
        <div className="rounded-lg border p-4">
          <div className="flex flex-wrap items-center justify-between gap-2 pb-3 border-b">
            ...
            {!readOnly && (
              <div className="relative">
                <input type="file" id="mp-video-upload" accept="video/mp4" className="hidden" disabled={uploadingCategory !== null} onChange={(e) => handleUpload(e, "VIDEO")} />
                <Button size="sm" variant="outline" asChild disabled={uploadingCategory !== null}>
                  <label htmlFor="mp-video-upload" className="cursor-pointer flex items-center gap-1"><Upload className="h-3.5 w-3.5" /> Unggah Video</label>
                </Button>
              </div>
            )}
          </div>
