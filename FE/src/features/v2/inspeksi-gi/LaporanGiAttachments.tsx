...
interface LaporanGiAttachmentsProps {
  laporanGiId?: string;
  readOnly?: boolean;
}

export function LaporanGiAttachments({ laporanGiId, readOnly = false }: LaporanGiAttachmentsProps) {
  ...
  const fetchAttachments = async (silent = false) => {
    if (!laporanGiId) return;
    if (!silent) setLoading(true);
    try {
      const res = await apiClient.get(`/v1/laporan-gi/${laporanGiId}/attachments`);
  ...
  // Poll attachments if any is PROCESSING
  useEffect(() => {
    fetchAttachments();
  }, [laporanGiId]);
  ...
  useEffect(() => {
    ...
    if (hasProcessing) {
      if (!pollingRef.current) {
        console.log("[Attachments] Starting polling for processing video...");
      ...
      if (pollingRef.current) {
        console.log("[Attachments] Stopping polling (no processing items left).");
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>, category: "SLD" | "LOGGER" | "FOTO" | "VIDEO") => {
    ...
    // If SLD, check if there is an existing SLD, delete it first
    if (category === "SLD") {
      const existingSld = attachments.find(a => a.category === "SLD");
      if (existingSld) {
        try {
          await apiClient.delete(`/v1/laporan-gi/attachments/${existingSld.id}`);
        } catch (err) {
    ...
    const uploadSingle = async (file: File) => {
      // Validate file size is under 150MB
      ...
      try {
        await apiClient.post(`/v1/laporan-gi/${laporanGiId}/attachments`, formData, {
          headers: {
  ...
  const handleDelete = async (attId: string) => {
    if (window.confirm("Apakah Anda yakin ingin menghapus lampiran ini?")) {
      try {
        await apiClient.delete(`/v1/laporan-gi/attachments/${attId}`);
        toast.success("Lampiran berhasil dihapus");
        fetchAttachments(true);
        // Revoke URL if playing
  ...
  const handleDownload = async (attId: string, originalName: string) => {
    try {
      ...
      const res = await apiClient.get(`/v1/laporan-gi/attachments/${attId}/download`, {
        responseType: "blob"
      });
  ...
  const handlePlayVideo = async (attId: string) => {
    if (videoPlayUrl[attId]) return; // Already loaded
  ...
  return (
    <Card className="w-full">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
        <CardTitle className="text-base font-bold">Dokumen &amp; Lampiran Teknis GI</CardTitle>
            ...
            {!readOnly && (
              <div className="relative">
                ...
                <Button 
                  ...
                  disabled={uploadingCategory !== null}
                >
                  <label htmlFor="sld-upload" className="cursor-pointer flex items-center gap-1">
                    <Upload className="h-3.5 w-3.5" /> Unggah SLD
                  </label>
                </Button>
              </div>
            )}
              ...
              <div className="flex items-center justify-between rounded border p-2 bg-muted/40">
                <div className="flex items-center gap-3">
                  <div className="h-12 w-12 rounded border bg-background overflow-hidden flex items-center justify-center">
                    <AttachmentPreviewImage attId={sldFile.id} alt="SLD" />
                  </div>
                  <div>
                    <p className="text-xs font-medium truncate max-w-[200px] sm:max-w-sm" title={sldFile.originalName}>
                      {sldFile.originalName}
                    </p>
                    <p className="text-[10px] text-muted-foreground">{formatSize(sldFile.fileSize)}</p>
                  </div>
                </div>
                <div className="flex gap-1.5">
                  <Button size="icon" variant="ghost" className="h-8 w-8 text-blue-600" onClick={() => handleDownload(sldFile.id, sldFile.originalName)}>
                    <Download className="h-4 w-4" />
                  </Button>
                  {!readOnly && (
                    <Button size="icon" variant="ghost" className="h-8 w-8 text-destructive" onClick={() => handleDelete(sldFile.id)}>
            ...
            {!readOnly && (
              <div className="relative">
                <input
                  ...
                  className="hidden"
                  disabled={uploadingCategory !== null}
                  onChange={(e) => handleUpload(e, "LOGGER")}
                />
                <Button 
                  size="sm" 
                  variant="outline" 
                  ...
                  disabled={uploadingCategory !== null}
                >
                  <label htmlFor="logger-upload" className="cursor-pointer flex items-center gap-1">
                    <Upload className="h-3.5 w-3.5" /> Unggah Logger
                  </label>
                </Button>
              </div>
            )}
              ...
              loggerFiles.map(file => (
                <div key={file.id} className="flex items-center justify-between rounded border p-2 bg-muted/40">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <p className="text-xs font-medium truncate max-w-[200px] sm:max-w-sm" title={file.originalName}>
                        {file.originalName}
                      </p>
                  ...
                  <div className="flex gap-1.5">
                    <Button size="icon" variant="ghost" className="h-8 w-8 text-blue-600" onClick={() => handleDownload(file.id, file.originalName)}>
                      <Download className="h-4 w-4" />
                    </Button>
                    {!readOnly && (
                      <Button size="icon" variant="ghost" className="h-8 w-8 text-destructive" onClick={() => handleDelete(file.id)}>
                        <Trash2 className="h-4 w-4" />
                      </Button>
                    )}
                  </div>
            ...
            {!readOnly && (
              <div className="relative">
                <input
                  ...
                  multiple
                  className="hidden"
                  disabled={uploadingCategory !== null || photoFiles.length >= 20}
                ...
                <Button 
                  ...
                  disabled={uploadingCategory !== null || photoFiles.length >= 20}
                >
                  <label htmlFor="photo-upload" className="cursor-pointer flex items-center gap-1">
          ...
          <div className="mt-3">
            {uploadingCategory === "FOTO" && (
              <div className="space-y-1 py-2 mb-3">
                <div className="flex justify-between text-xs">
                  <span>Mengunggah beberapa foto...</span>
                  <span>{uploadProgress}%</span>
                </div>
                <Progress value={uploadProgress} className="h-1.5" />
              </div>
                ...
                {photoFiles.map(file => (
                  <div key={file.id} className="relative group rounded border overflow-hidden bg-muted/40 aspect-square flex flex-col justify-between">
                    <div className="relative flex-1 w-full overflow-hidden flex items-center justify-center bg-background cursor-pointer" onClick={() => handleDownload(file.id, file.originalName)}>
                      <AttachmentPreviewImage attId={file.id} alt="Foto" />
                    ...
                    <div className="bg-background border-t p-1 flex items-center justify-between text-[10px]">
                      ...
                      {!readOnly && (
                        <button className="text-destructive hover:text-red-700" onClick={() => handleDelete(file.id)}>
            ...
            {!readOnly && (
              <div className="relative">
                ...
                <Button 
                  ...
                  disabled={uploadingCategory !== null}
                >
                  <label htmlFor="video-upload" className="cursor-pointer flex items-center gap-1">
                    <Upload className="h-3.5 w-3.5" /> Unggah Video
                  </label>
                </Button>
              </div>
            )}
              ...
              videoFiles.map(file => (
                <div key={file.id} className="rounded border p-3 bg-muted/40 space-y-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div>
                      <p className="text-xs font-semibold truncate max-w-xs sm:max-w-md" title={file.originalName}>
                        {file.originalName}
                      </p>
                      <p className="text-[10px] text-muted-foreground">{formatSize(file.fileSize)}</p>
                    </div>
                    ...
                    <Alert variant="destructive" className="py-2 px-3 text-xs">
                      ...
                      <div>
                        ...
                        <AlertDescription className="text-[10px] mt-0.5">
                          {file.errorMessage || "Terjadi kesalahan kompresi video di backend."}
                        </AlertDescription>
                      </div>
                    </Alert>
                  )}
                    ...
                    <div className="rounded border bg-background overflow-hidden relative max-w-md mx-auto aspect-video flex items-center justify-center">
                      {videoPlayUrl[file.id] ? (
                        ...
                        <div className="relative w-full h-full group cursor-pointer flex items-center justify-center" onClick={() => handlePlayVideo(file.id)}>
                          <AttachmentPreviewVideoPoster attId={file.id} />
...
// Subcomponents to handle scoped auth blob-to-url image loads
function AttachmentPreviewImage({ attId, alt }: { attId: string; alt: string }) {
  const [src, setSrc] = useState<string>("");
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let active = true;
    apiClient.get(`/v1/laporan-gi/attachments/${attId}/download`, { responseType: "blob" })
      .then(res => {
        if (active) {
          const url = window.URL.createObjectURL(res.data);
          setSrc(url);
...
function AttachmentPreviewVideoPoster({ attId }: { attId: string }) {
  const [src, setSrc] = useState<string>("");
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let active = true;
    apiClient.get(`/v1/laporan-gi/attachments/${attId}/thumbnail`, { responseType: "blob" })
      .then(res => {
        if (active) {
          const url = window.URL.createObjectURL(res.data);
          setSrc(url);
