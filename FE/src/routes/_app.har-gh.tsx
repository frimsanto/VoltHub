...
import { HarGhForm } from "@/features/v2/har-gh/HarGhForm";
...
function HarGhPage() {
  ...
  const showingDetail = matches.some((m) => m.routeId === "/_app/har-gh/$id");
  ...
  const [importedDetail, setImportedDetail] = useState<HarGhRow | null>(null);

  // ?wo= dievaluasi ulang tiap kali berubah (navigasi antar-WO tidak selalu remount
  // komponen ini), dan diverifikasi terhadap laporan existing agar deterministik:
  // sudah ada → redirect ke edit; belum ada → form create. Tidak pernah diam-diam jatuh ke list.
  const woLookupQ = harGh.useList({ workOrderId: woParam }, { enabled: !!woParam });
  ...
  useEffect(() => {
    ...
    if (existingForWo) {
      navigate({ to: "/har-gh/$id", params: { id: existingForWo.id }, replace: true });
      return;
    }
    setMode("create");
  }, [woParam, woLookupQ.isLoading, existingForWo, navigate]);

  const query = harGh.useList({
  ...
  const columns = useMemo<ColumnDef<HarGhRow>[]>(
    () => [
      {
        header: "Tanggal",
          ...
          <RowActions
            onView={() =>
              row.original.isImported
                ...
                : navigate({ to: "/har-gh/$id", params: { id: row.original.id } })
            }
          />
        ),
      },
  ...
  if (mode === "create" && isPetugas) {
    return (
      <div>
        <PageHeader
          title="Isi Laporan HAR GH"
          description="Laporan korektif Gardu Hubung (RTUPP2-5) — tertaut Work Order."
          actions={
        ...
        <HarGhForm
          ...
          onSubmit={async (values: CreateHarGh, intent) => {
            setSubmitting(true);
            try {
              const created = await v2Post<HarGhDetail, CreateHarGh>("/gh/har", values);
              if (intent === "SUBMITTED") {
              ...
              navigate({ to: "/har-gh/$id", params: { id: created.id } });
            } catch (e) {
  ...
  return (
    <div>
      <PageHeader
        title="Laporan HAR GH"
        description={
          isPetugas
            ? "Riwayat Laporan HAR GH Anda (RTUPP2-5). Membuat laporan baru dilakukan dari Work Order."
            : "Riwayat & tinjauan Laporan HAR GH dari tim lapangan (RTUPP2-5)."
        }
      />
      <DataTable
        columns={columns}
        ...
        onRowClick={(row) =>
          row.isImported ? setImportedDetail(row) : navigate({ to: "/har-gh/$id", params: { id: row.id } })
        }
        toolbar={
          <ListToolbar
