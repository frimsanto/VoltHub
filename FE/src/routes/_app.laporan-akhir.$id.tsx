...
import { getById, validate, remove, type LaporanAkhir } from "@/lib/api/laporanAkhir";
import { LaporanAkhirDetailView } from "@/features/report";
...
export const Route = createFileRoute("/_app/laporan-akhir/$id")({
  beforeLoad: () => requireV2Role(FIELD_ONLY_ROLES),
  component: LaporanAkhirDetail,
  head: () => ({ meta: [{ title: "Detail Laporan Akhir — VoltHub" }] }),
});

function LaporanAkhirDetail() {
  ...
  const handlePreview = (attachment: any) => {
    // For now, just show category since attachment type is limited
      ...
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          ...
          <Button
            variant="outline"
            className="mt-4"
            onClick={() => navigate({ to: "/laporan-akhir" })}
          >
            Kembali
          </Button>
        </div>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={`Laporan Akhir - ${laporan.reportId}`}
        description="Detail laporan akhir pekerjaan"
        actions={
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={() => navigate({ to: "/laporan-akhir" })}>
              <ArrowLeft className="size-4 mr-2" />
              Kembali
            </Button>
            <ReportExportMenu sourceType="LAPORAN_AKHIR" sourceId={id} variant="outline" size="sm" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <LaporanAkhirDetailView laporan={laporan} onPreviewAttachment={handlePreview} />
        </div>
