...
import { HarMpForm, type SubmitIntent } from "@/features/v2/har-mp/HarMpForm";
...
export const Route = createFileRoute("/_app/har-mp/$id")({
  beforeLoad: () => requireAuth(),
  component: HarMpDetailPage,
});
...
function HarMpDetailPage() {
  const { id } = Route.useParams();
  const role = useV2Role();
  const user = useAuthStore((s) => s.user);
  const { data, isLoading } = harMp.useOne(id);
  const submitM = useSubmitHarMp();
  const validateM = useValidateHarMp();
  ...
  if (isLoading || !data) {
    return (
      <div>
        <PageHeader title="Detail Laporan HAR MP" actions={<BackBtn />} />
  ...
  if (editing) {
    return (
      <div>
        <PageHeader
          title={`Revisi HAR MP · ${data.location?.name ?? "—"}`}
        ...
        <HarMpForm
          ...
          onSubmit={async (values: CreateHarMp, intent: SubmitIntent) => {
            setSaving(true);
            try {
              await v2Put<HarMpDetail, CreateHarMp>(`/mp/har/${id}`, values);
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title={`Laporan HAR MP · ${data.location?.name ?? "—"}`}
      ...
      <MpReportDetail variant="HAR" data={data as unknown as Record<string, unknown>} />
      ...
      <MpAttachments basePath="/v1/mp/har" parentId={id} readOnly={!editable} />
...
function BackBtn() {
  ...
  return (
    <Button variant="outline" onClick={() => navigate({ to: "/har-mp" })}>
