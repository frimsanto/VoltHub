...
import {
  inspeksiGh,
  useSubmitInspeksiGh,
  useValidateInspeksiGh,
  type CreateInspeksiGh,
  type InspeksiGhDetail,
} from "@/features/v2/inspeksi-gh/resource";
import { InspeksiGhForm, type SubmitIntent } from "@/features/v2/inspeksi-gh/InspeksiGhForm";
...
export const Route = createFileRoute("/_app/inspeksi-gh/$id")({
  beforeLoad: () => requireAuth(),
  component: InspeksiGhDetailPage,
});
...
function InspeksiGhDetailPage() {
  const { id } = Route.useParams();
  const role = useV2Role();
  const user = useAuthStore((s) => s.user);
  const { data, isLoading } = inspeksiGh.useOne(id);
  const submitM = useSubmitInspeksiGh();
  const validateM = useValidateInspeksiGh();
  ...
  if (isLoading || !data) {
    return (
      <div>
        <PageHeader title="Detail Inspeksi GH" actions={<BackBtn />} />
  ...
  if (editing) {
    return (
      <div>
        <PageHeader
          title={`Revisi Inspeksi GH · ${data.location?.name ?? "—"}`}
        ...
        <InspeksiGhForm
          ...
          onSubmit={async (values: CreateInspeksiGh, intent: SubmitIntent) => {
            setSaving(true);
            try {
              await v2Put<InspeksiGhDetail, CreateInspeksiGh>(`/gh/inspeksi/${id}`, values);
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title={`Inspeksi GH · ${data.location?.name ?? "—"}`}
      ...
      <GhReportDetail variant="INSPEKSI" data={data as unknown as Record<string, unknown>} />
      ...
      <GhAttachments basePath="/v1/gh/inspeksi" parentId={id} readOnly={!editable} />
...
function BackBtn() {
  ...
  return (
    <Button variant="outline" onClick={() => navigate({ to: "/inspeksi-gh" })}>
